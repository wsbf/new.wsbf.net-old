...
$qu = sprintf("SELECT libcart.cartID, libcart.start_date, libcart.end_date, libcart.play_mask, libcart.title, libcart.issuer, def_cart_type.type, libcart.filename FROM `libcart`, `def_cart_type` WHERE libcart.start_date <= '%s' AND (libcart.end_date >= '%s' OR libcart.end_date IS NULL OR libcart.end_date = '0000-00-00 00:00:00') AND def_cart_type.type LIKE '%s' AND def_cart_type.cart_typeID = libcart.cart_typeID order by libcart.cartID DESC", $now_ts, $now_ts, $type);
