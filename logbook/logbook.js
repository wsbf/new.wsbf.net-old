...
$(function() {
	...
	/**** Sign Off and go to automation (i.e. sign in as Automation) ****/
...
/* refreshCurrShow 
 * refreshes the stuff at the top (showname/id/etc)
 */
...
//				$("#listeners").text("Listeners: "+row[6]);
	...
	showTimer = setTimeout("refreshCurrShow()", 10000);
}

/** refreshListeners() - refreshes the listener count, dumbass */
function refreshListeners(){
	clearTimeout(listenerTimer);
	
	$.ajax({
		type: "GET",
		url: "logging_getlisteners.php",
		cache: false,
		success: function(html){
			if(html == '-1'){	// can't get listeners
				$("#listeners").text("Something's wrong...");
			}
			else{
				$("#listeners").text("Listeners: " + html);
			}
		}
	...
	listenerTimer = setTimeout("refreshListeners()", 3000);	
}
