...
if(!session_id())	session_start();
	...
	$album_query = sprintf("SELECT album_name, num_discs, artistID, labelID, genre, general_genreID, album_code, rotationID FROM libalbum WHERE albumID = '%d'", $albumID);
	...
	echo "<head><title>WSBF CD Review System</title>";

	echo "<style>#top {text-align:right;}  #center {text-align:center;}</style>";

	echo "</head>";

	echo "If you change your mind, you can <a href=\"library.php\">go back</a>, or you can <a href=\"logout.php\">log out</a>.<br>";
	...
	echo "<form action='submit_review.php' method='POST'><input type='hidden' name='albumID' value='$albumID'/><input type='hidden' name='rotationID' value='$rotationID'/><input type='hidden' name='album_code' value='$album_code'/><table>\n";
	echo "<tr><td><div id=\"top\">Album Artist:</div></td><td> <INPUT TYPE = \"Text\" SIZE ='30' VALUE =\"$artist\" NAME = \"artist\"><--------</td><td style=\"vertical-align:top\" rowspan=\"7\">This is the album artist, if it's various artists, put \"Various Artists\" and make the track artist the name of the artist who performed each track. If one artist performed all the tracks, leave the track artists alone. If an album is by an artist, but has tracks featuring other artists, you can change the track artist to say \"Album Artist feat. Another Artist.\" If an album has remixes, and titles like \"Track Name remixed by Artist Name,\" you should enter the track as \"Track Name (Remix)\" and make the track artist the name of the artist who did the remix.</td></tr>\n";
	echo "<tr><td><div id=\"top\">Album:</div></td><td> <INPUT TYPE = \"Text\" SIZE ='30' VALUE =\"$album_name\" NAME = \"album\"></td></tr>\n";
	echo "<tr><td><div id=\"top\">Label:</div></td><td> <INPUT TYPE = \"Text\" SIZE ='30' VALUE =\"$label\" NAME = \"label\"></td></tr>\n";
	echo "<tr><td><div id=\"top\">Genre:</div></td><td> <INPUT TYPE = \"Text\" SIZE ='30' VALUE =\"$genre\" NAME = \"genre\"></td></tr>\n";
	...
	echo "<tr><td><div id=\"top\">General Genre:</div></td><td><select $disabled name=\"general_genreID\">";
	...
	echo "<tr><td><div id=\"top\">Username:</div></td><td> <INPUT TYPE = \"Text\" $disabled SIZE ='40' VALUE =\"$username\" NAME = 'username' ></td></tr>\n";
	echo "<tr><td><div id=\"top\">Reviewer:</div></td><td> <INPUT TYPE = \"Text\" SIZE ='40' VALUE =\"$pref_name\" NAME = \"display\"></td></tr>\n";
	echo "</table>\n";
	...
	$track_total = 0;
	echo "<table>\n";
	echo "<tr><th>Disc No.</th><th>Track No.</th><th>Track Name</th><th>Track Artist</th><th>Track Status</tr>\n";
	...
	while($row = mysql_fetch_array($track, MYSQL_NUM)){
		...
		echo "<tr><td><div id=\"center\">$disc_num</div></td><td><div id=\"center\">$track_num</div></td>
...
Review: <br>
You have <span id=\"charLeft\"> </span> characters remaining for your review.<br>";


	echo "<textarea id=\"ta\" name='review' style=\"resize: none;\" cols=\"65\" rows=\"12\">$review</textarea>";
