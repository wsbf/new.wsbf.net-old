...
foreach($bowls as $num => $bowl) {
	...
	foreach($bowl as $row) {
			...
			$djquery = sprintf("SELECT preferred_name FROM users WHERE username='%s'", $username);
			$qdj = mysql_query($djquery) or die(mysql_error());
			while($dj = mysql_fetch_array($qdj, MYSQL_ASSOC)) {
				$name = $dj['preferred_name'];
			}
			if($name) echo "<tr><td>".$row['id']."</td><td>".$name."</td><td>".$row['average']."</td><td></tr>";
	else echo "<tr><td>".$row['id']."</td><td>".$row['username']."</td><td>".$row['average']."</td><td></tr>";
	}
